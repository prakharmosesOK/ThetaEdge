...
const GameDetails = ({ game, setGame, timeToDisplay, setGameParticipants }) => {
  ...
  const handlePaymentClicked = async () => {
    ...
    try {
      ...
      alert('Payment successfull! Genarate the API keys and enter in your OBS studio and start the Game.');
    } catch (error) {
  ...
  const generateStreamDetails = async () => {
    ...
    const rxStreamResponse = await _contract.UpdatePlayerStreamLink(game.gameId, `https://live5.thetavideoapi.com/hls/live/2015862/${output.assignments[0].stream_server}/1721935096573/master.m3u8`);
    await rxStreamResponse.wait();
    setGameParticipants((prev) => {
      let newDetails = [...prev];
      const index = newDetails.findIndex((item) => item.playerAddress === account);
      newDetails[index].streamLink = `https://live5.thetavideoapi.com/hls/live/2015862/${output.assignments[0].stream_server}/1721935096573/master.m3u8`;
    });
    ...
    <Box p={5}>
      ...
      <Flex gap="2em">
        {game.bIsInvite === true && <Input
          ...
          color="white"
        />}
        {timeToDisplay[0] < 3 ? (!game.hasPlay && !game.hasStream && 
            ...
            <Text w="20em" mx="auto" textAlign="center" color="yellow">Game event has Ended!</Text>
          )}
      </Flex>
      {game.streamLink && <Flex flexDirection="column" alignItems="flex-start" justifyContent="center" w="45em" mx="auto">
        <Text color="#2acafa" textShadow="0 0 10px white"><strong>Stream Server:</strong> {game.streamDetails.stream_server}</Text>
        <Text color="#2acafa" textShadow="0 0 10px white"><strong>Stream key:</strong> {game.streamDetails.stream_key}</Text>
      </Flex>}

      <Flex flexDirection="row" width="50em" gap="5em" mx="auto" alignItems="center" justifyContent="space-evenly" my="2em">
        <Button onClick={generateStreamDetails} w="20em">Generate Stream Details</Button>
        {((game.bIsMultiplayer && timeToDisplay[0] === 1) || (!game.bIsMultiplayer && timeToDisplay[0] === 2)) ?
          (!showGameFrame ?
            <Button onClick={handleStartGame} w="20em">Go to Game</Button> :
            <Button onClick={() => setShowGameFrame(false)} w="20em">Hide</Button>
          ) : null}
      </Flex>
