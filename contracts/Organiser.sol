...
contract Organiser {
    ...
    mapping(address => uint256[]) public organiserGames;
    GameData[] public gamesList;
    mapping(address => uint256[]) PlayerData;
    ...
    function UploadGames(
        ...
        organiserGames[msg.sender].push(newGameId);
    }
    ...
    function JoinGame(
        ...
        gamesList[_gameId].playersJoined.push(Player({
        ...
        PlayerData[msg.sender].push(_gameId);
    ...
    function UpdateLeaderBoard(uint256 _gameid,uint256 score) public {
        for(uint256 i=0 ; i < gamesList[_gameid].playersJoined.length;i++){
            if(gamesList[_gameid].playersJoined[i].playerAddress == msg.sender){
    ...
    function GetReward(uint256 _gameid) public {
        ...
        for(uint256 i=0 ; i < len ; i++){
            if(gamesList[_gameid].playersJoined[i].playerAddress != msg.sender){
                if(gamesList[_gameid].playersJoined[i].leaderboardScore > MyScore){
                    position += 1;
                }
                else if(gamesList[_gameid].playersJoined[i].leaderboardScore == MyScore){
                    if(!tiebraker){
                        position +=1;
                    }
                }
        ...
        if(position == 1){
            ...
            if(position >= 4 && position <= newlen + 3){
                payable(msg.sender).transfer((15*totalPrizeMoney) / (100 * newlen));
            } 
        }
    }
}
